...
const COMMENTS = [
  { id: 'cm01', postId: 'j01', name: 'Noah', text: '我也在找 9 号线附近的，方便一起看房吗？' },
  { id: 'cm02', postId: 'j01', name: 'Iris', text: '阳台朝南真不错，我们周末约吗？' },
];

const FOLLOWS = [
  { followerId: 'p01', followeeId: 'p03' },
  { followerId: 'p03', followeeId: 'p01' },
];
...
exports.main = async (event, context) => {
  try {
    // 清空旧数据
    await db.collection('harmony-users').where({}).remove();
    await db.collection('harmony-posts').where({}).remove();
    await db.collection('harmony-comments').where({}).remove();
    await db.collection('harmony-follows').where({}).remove();
    await db.collection('harmony-conversations').where({}).remove();

    // 批量导入
    const userRes = await db.collection('harmony-users').add(PEOPLE);
    const postRes = await db.collection('harmony-posts').add(POSTS);
    const commentRes = await db.collection('harmony-comments').add(COMMENTS);
    const followRes = await db.collection('harmony-follows').add(FOLLOWS);
    const convRes = await db.collection('harmony-conversations').add(CONVERSATIONS);

    return {
      code: 0,
      message: '导入成功',
      data: {
        users: userRes.affectedDocs,
        posts: postRes.affectedDocs,
        comments: commentRes.affectedDocs,
        follows: followRes.affectedDocs,
        conversations: convRes.affectedDocs,
      },
    };
  } catch (e) {
    return {
